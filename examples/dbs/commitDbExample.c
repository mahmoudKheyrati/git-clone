...
//// write to db
    ...
    struct CommitFileEntry entry= {.id=id,.fileAddress="changedFile1.txt",.status=ADD_NEW_FILE};
    struct CommitFileEntry entry2= {.id=id,.fileAddress="changedFile2.txt",.status=REMOVED_FILE};
    struct CommitFileEntry entry3= {.id=id,.fileAddress="changedFile3.txt",.status=CHANGED_FILE};
...
void readFromCommitDbExample() {
    ...
    for (int i = 0; i < list2->length; ++i) {
        ...
        print("%s %s %s %i\n", current.id, current.fileAddress, current.date, current.status);
    }
}
